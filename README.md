...
- **자동 수집 또는 분석 엑셀 업로드** — somoim API(`/api/articles`·`/api/photos`·`/api/group`)에서 게시글·사진·활성 멤버·**가입인사 전체**를 모으거나, 이전에 받은 분석 엑셀을 그대로 올리면 API 호출 없이 즉시 분석.
- **🪪 가입인사 자동 매핑** — 가입인사 본문의 `이름 : XXX` 패턴을 그룹 시작부터 전부 스캔해 (실명 → 활성 멤버 닉네임) 매핑을 자동 구축. 미매칭 이름 해소 작업량을 크게 줄여줍니다.
- **2단계 보정 워크플로** — ① **미매칭 이름 정리**(후기 본문에서 추출된 이름 중 활성 멤버 명단과 일치하지 않는 것을 드롭다운 3택으로 해소, 가입인사 자동 매핑은 기본값으로 미리 채움) → ② 분류·참석자 보정 → ③ 인사이트. 모든 보정은 `st.data_editor`로 직접 편집.
- **인사이트 대시보드** (탭 구성)
...
- **분석 엑셀 = 결과 + 원본 + 매핑** — 12개 가시 시트(인사이트) + 숨김 시트(원본 게시글·사진·활성 멤버·탈퇴 멤버·이름 매핑·가입인사 자동 매핑)를 한 파일에. 그 엑셀을 그대로 다시 업로드하면 같은 분석을 즉시 다시 볼 수 있습니다.
