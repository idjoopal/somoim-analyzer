...
- **자동 수집** — 공지·후기·가입인사 게시글과 업로드된 사진을 모아옵니다.
- **분류 검토·보정** — 자동 분류가 애매한 출사 공지(카테고리·출사일·진행/취소)를 표에서 직접 고친 뒤 분석을 진행합니다.
- **인사이트 대시보드** (탭 구성)
...
### 배포된 앱
앱 URL 접속 → 년도 선택(필요하면 "월 단위 분석" 체크) → **분석 시작** → 분류 검토 후 **분석 진행** → **엑셀 다운로드**.

> 최초 수집은 약 40초 걸립니다. 같은 기간 재실행은 세션 캐시로 즉시 표시됩니다.
