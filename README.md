...
- **자동 수집 또는 분석 엑셀 업로드** — somoim API에서 모으거나, 이전에 받은 분석 엑셀을 그대로 올리면 API 호출 없이 즉시 분석.
...
- **분석 엑셀 = 결과 + 원본** — 11개 가시 시트(인사이트) + 4개 숨김 시트(원본 게시글·사진·확정 마스터)를 한 파일에. 그 엑셀을 그대로 다시 업로드하면 같은 분석을 즉시 다시 볼 수 있습니다.

## 사용법

### 배포된 앱
사이드바에서 **API 수집** → 년/월 → **분석 시작**(첫 수집 ~40초), 또는 이전에 받은 **분석 엑셀**을 업로드. 본문에서 ① 마스터 확정 → ② 분류·참석자 보정 → ③ 인사이트. 사이드바에서 **엑셀 다운로드**.

> 분석이 끝나면 엑셀 한 파일만 받습니다. 다음에 그 엑셀을 다시 올리면 API 호출 없이 즉시 같은 분석이 열립니다.
