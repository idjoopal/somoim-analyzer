    ...
    with st.spinner("구글 시트에서 데이터를 읽는 중…"):
        try:
            ...
            st.error(
                ...
                "위에서부터 확인해 주세요.\n\n"
                "1. **다시 실행** — 구글 API가 잠깐 막히면(429·503) 다시 "
                "시도하는 것만으로 풀립니다. 우상단 **⋮ → Rerun**(단축키 `R`) "
                "이나 브라우저 새로고침 둘 다 같습니다.\n"
                "2. **폴더 공유** — `[gsheets] folder_id`의 폴더가 서비스 계정 "
                "이메일에 **편집자**로 공유돼 있는지 봅니다. 공유가 풀리면 "
                "폴더가 아예 없는 것처럼 보입니다.\n"
                "3. **캐시 비우기 · 앱 재시작** — 자격증명을 갈았거나 시트를 "
                "옮겼다면 **새로고침으로는 안 풀립니다.** 앱이 붙잡고 있는 "
                "연결이 서버에 캐시돼 세션을 넘어 살아남기 때문입니다. "
                "우상단 **⋮ → Clear cache**(단축키 `C`), 그래도 그대로면 "
                "우하단 **Manage app → ⋮ → Reboot app**으로 앱을 다시 "
                "띄웁니다.\n"
