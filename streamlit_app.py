    ...
    with st.spinner("구글 시트에서 데이터를 읽는 중…"):
        try:
            ...
            st.error(
                ...
                "위에서부터 확인해 주세요.\n\n"
                "1. **다시 읽기** — 구글 API가 잠깐 막히면(429·503) 다시 "
                "시도하는 것만으로 풀립니다. 아래 버튼이나 브라우저 새로고침 "
                "둘 다 같습니다.\n"
                "2. **폴더 공유** — `[gsheets] folder_id`의 폴더가 서비스 계정 "
                "이메일에 **편집자**로 공유돼 있는지 봅니다. 공유가 풀리면 "
                "폴더가 아예 없는 것처럼 보입니다.\n"
                "3. **연결 다시 맺기** — 자격증명을 갈았거나 시트를 옮겼다면 "
                "**브라우저 새로고침으로는 안 풀립니다.** 앱이 붙잡고 있는 "
                "연결이 서버에 캐시돼 세션을 넘어 살아남기 때문입니다. 아래 "
                "버튼(없으면 앱 메뉴의 **Reboot app**)으로 다시 맺습니다.\n"
                "4. **탭 이름** — 시트에서 탭을 지웠거나 이름을 바꿨다면 "
                f"되돌립니다(raw: {' · '.join(RAW_TABS)}).\n\n"
                "다 맞는데도 그대로면 사이드바 **📥 수집**으로 다시 받아 "
                "시트를 새로 세울 수 있습니다.",
                icon="⚠️",
            )
            # 두 버튼은 **하는 일이 다르다.** 왼쪽은 읽기만 다시 하고, 오른쪽은
            # 캐시된 연결(자격증명·시트 위치)까지 버린다. 오른쪽이 필요한
            # 상황을 사용자가 스스로 알아내기는 어려우므로 둘 다 내놓는다.
            c1, c2 = st.columns(2)
            if c1.button("🔄 다시 읽기", width="stretch"):
                st.rerun()
            if c2.button("♻️ 연결 다시 맺기", width="stretch",
                         help="자격증명과 시트 위치를 다시 잡습니다. "
                              "앱 재시작(Reboot)과 같은 효과입니다."):
                _open_stores.clear()
                st.rerun()
